...
| D7 | **LLM：豆包 2.1 turbo + 联网搜索，OpenAI 兼容适配器** | 2026-09-01 从 DeepSeek 切换：豆包支持方舟托管 web_search，主播可搜索核实后聊作品背景；OpenAI 兼容协议，换供应商=改配置。 |
...
│  ├─ LLM 适配器  豆包 2.1 turbo（OpenAI 兼容 + 联网搜索）→ 串场文本           │
...
上下文现含近期已播口播（对话史）；主指令保持短；不再注入 seeds / guardrails / 世界书（2026-09-01 停用，改联网搜索核实）。联网搜索用于核实作品与游戏背景，不用于播报实时资讯（FR-036/037）。
...
- OpenAI 兼容协议一个客户端通吃：豆包（默认，方舟）/ DeepSeek / Qwen / GLM / Kimi / OpenAI。`base_url + api_key + model` 三行配置。
- 联网搜索（2026-09-01 接入）：方舟模型支持顶层 `web_search: {enable: true}`，由方舟托管搜索并回填，一次调用完成；适配器 `webSearch` 开关控制。
- 手写世界书已停用（2026-09-01）：维护者手写设定易错（实测污染模型），改为模型联网搜索核实；`lore-files.ts` 保留备用。
