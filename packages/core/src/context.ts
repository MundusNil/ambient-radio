...
/** 内置起头灵感（当 station.config.json 未提供 interlude.seeds 时兜底）；模型可用可不用 */
...
/** 内置起头护栏（当 station.config.json 未提供 interlude.guardrails 时兜底） */
const DEFAULT_GUARDRAILS: readonly string[] = [
  '不要用「现在是周X的…」这类报时式开场',
  '不要逐首报幕，也不要预告接下来要放什么',
  '不要反问听众（例如「你今晚听得开心吗」）',
...
export function buildSegmentPrompt(ctx: SegmentPromptContext): SegmentPrompt {
  ...
  // 起头：可选灵感 + 护栏，不强制单一角度（信任模型自由发挥，活人感优先）
  const seeds = ctx.interlude?.seeds?.length ? ctx.interlude.seeds : DEFAULT_SEEDS;
  const guardrails = ctx.interlude?.guardrails?.length
    ? ctx.interlude.guardrails
    : DEFAULT_GUARDRAILS;
  const showSeeds = ctx.kind === 'interlude' || ctx.kind === 'topic';
  if (showSeeds && seeds.length > 0) {
    lines.push('可选的起头灵感（可用可不用，化成你自己的口语，切勿照搬原文）：');
    seeds.forEach((s, i) => {
      lines.push(`  ${i + 1}. ${s}`);
    });
  }
  if (guardrails.length > 0) {
    lines.push(`起头护栏（务必遵守）：${guardrails.join('；')}。`);
  }
  lines.push('');
